...
void statusbar_push_temporary_msg(const gchar* status) {}
...
cf_read(capture_file* cf, gboolean reloading)
{
    ...
    if (cf->first_displayed != 0) {
        //peter packet_list_select_row_from_data(NULL);
    }
    ...
    Buffer* buf, int* err)
{
    ...
    *err = 0;
     ...
     */
    //peter if (!cf->current_frame && !packet_list_multi_select_active())
    //peter     packet_list_select_row_from_data(NULL);
    ...
    wtap_rec* rec, Buffer* buf, gboolean add_to_packet_list)
{
    ...
    //peter if (add_to_packet_list) {
        /* We fill the needed columns from new_packet_list */
    //peter     packet_list_append(cinfo, fdata);
    //peter }
...
rescan_packets(capture_file* cf, const char* action, const char* action_item, gboolean redissect)
{
    ...
    //peter if (!add_to_packet_list)
    //peter     packet_list_recreate_visible_rows();
    ...
    void* criterion, search_direction dir)
{
    ...
    if (new_fd != NULL) {
        ...
        //peter found_row = packet_list_select_row_from_data(new_fd);
